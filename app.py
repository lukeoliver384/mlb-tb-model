...
st.markdown(
    ...
    + (
        ...
        'background:#4C8DFF;margin-right:6px;"></span>SLATE LOADED</div>'
        if st.session_state.get("slate") else
        '<div style="flex:0 0 auto;align-self:center;font-size:0.72rem;font-weight:600;'
        'color:#8B949E;background:rgba(139,148,158,0.10);border:1px solid rgba(139,148,158,0.22);'
        'padding:5px 11px;border-radius:20px;letter-spacing:0.03em;">'
        '<span style="display:inline-block;width:6px;height:6px;border-radius:50%;'
        'background:#6E7681;margin-right:6px;"></span>NO SLATE LOADED</div>'
    )
    + '</div>',
...
with colB:
    _last_loaded = st.session_state.get("slate_loaded_at")
    if _last_loaded:
...
with tab_bet:
    if df is None or df.empty:
        ...
        def _lean_style(v):
            return {"Over": "background-color:#10362C;color:#5DE0BB;font-weight:600",
                    "Under": "background-color:#3A1414;color:#F85149;font-weight:600",
                    "No lean": "color:#7A828C"}.get(v, "color:#7A828C")
        try:
            dfv_sty = dfv.style.map(_lean_style, subset=["Lean"]) if hasattr(dfv.style, "map") \
                else dfv.style.applymap(_lean_style, subset=["Lean"])
        except Exception:
            dfv_sty = dfv
        st.dataframe(
            dfv_sty, use_container_width=True, hide_index=True,
            column_config={
